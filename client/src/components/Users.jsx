...
export const Users = () => {
  ...
  const { me, callUserFn, isUserBusy } = useContext(SocketContext);
  useEffect(() => {
    api
      .get(`/users`)
      .then((response) => {
  ...
  return (
    <div className="bg-blue-gray-400 flex justify-center  flex-col w-full p-10 ">
      {isUserBusy && <h2 className="text-sm text-center p-3 text-black bg-yellow-400">{isUserBusy}</h2>}
          ...
          if (user._id != me?._id)
            return (
              <li key={user._id} className="border-b-2 border-b-black flex justify-between p-7 items-center">
                ...
                <span
                  onClick={() => {
                    callUserFn({ receiverDBId: user._id, receiverName: user.name });
                    console.log({ receiverDBId: user._id, receiverName: user.name });
                  }}
